/*
 ...
 Driver.cpp implements Driver class and contains main method
 */
...
#define DATA_FILE_NAME "data.txt" //data file name
#define ARRAY_SIZE 1000 //maximum of elements in array
...
int Driver::sortMenu(){
	...
	cout << "4. Random sort (Insertion Sort, Bubble Sort or Quick Sort)" << endl;
	...
	while (selection != 2)
	{
		...
		case 1:
			...
			while (sortSelection != 1 && sortSelection != 2 && sortSelection != 3
				 && sortSelection != 4 && sortSelection != 5)
			{
			...
			if (sortSelection == 4)//random
			{
				sortSelection = rand() % 3 + 1;//1 or 2 or 3
			}
			...
			{//2. Bubble Sort
				cout << endl;
				sortAlg = new BubbleSort(data, length);
				sortAlg->sort();
				sortAlg->printArray();
				delete sortAlg;
